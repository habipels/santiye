...
class genel_rapor(models.Model):
    ...
    proje_ait_bilgisi = models.ForeignKey(CustomUser,verbose_name="Proje Ait Olduğu",blank=True,null=True,on_delete=models.SET_NULL)
    proje_santiye_Ait = models.ForeignKey(santiye,verbose_name="santiye Ait Olduğu",blank=True,null=True,on_delete=models.SET_NULL)
    blok_bilgisi = models.ForeignKey(bloglar,verbose_name="santiye Ait Olduğu",blank=True,null=True,on_delete=models.SET_NULL)
...
class genel_imalat(models.Model):
    ...
    imalat_calisan_sayisi = models.FloatField(blank=True,null=True,verbose_name="İmalat Çalışan Sayısı")
...
class rapor_bilgisi(models.Model):
    ...
    baslangic_tarihi = models.DateTimeField( null=True)
    bitis_tarihi = models.DateTimeField( null=True)
    rapor_dosyalari = models.FileField(upload_to='rapor_dosyalari/', verbose_name="Rapor Dosyası", blank=True, null=True)
    kayit_tarihi = models.DateTimeField(default=datetime.now, null=True)
    silinme_bilgisi = models.BooleanField(default=False)
    history = HistoricalRecords(user_model=settings.AUTH_USER_MODEL)