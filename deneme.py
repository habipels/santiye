...
def get_crypto_prices():
    ...
    # Ücretsiz bir proxy adresi kullanıyoruz
    proxies = {
        "http": "http://93.113.63.73:33100",  # Proxy adresinizi buraya ekleyin
        "https": "http://93.113.63.73:33100",  # Proxy adresinizi buraya ekleyin
    }

    try:
        # Zaman aşımını artırıyoruz (30 saniye gibi)
        response = requests.get(url, proxies=proxies, timeout=30, verify=certifi.where())
        response.raise_for_status()  # HTTP hatalarını yakalar
        all_prices = response.json()

        # USDT ile biten kripto paraları filtreliyoruz
